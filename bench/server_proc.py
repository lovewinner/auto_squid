...
async def _serve(config: dict):
    ...
    try:
        ...
        router = Router(ps, listen_host="127.0.0.1", listen_port=config["router_port"],
                        ...
                        policies=config.get("policies"),  # 策略路由(P1)
                        adaptive_ttl=config.get("adaptive_ttl", False),
                        adaptive_ttl_min=config.get("adaptive_ttl_min", 60.0),
                        adaptive_ttl_max=config.get("adaptive_ttl_max", 1800.0),
                        switch_damping=config.get("switch_damping", False),
                        switch_damping_min_wins=config.get("switch_damping_min_wins", 2),
                        switch_damping_ratio=config.get("switch_damping_ratio", 0.8),
                        switch_damping_abs_ms=config.get("switch_damping_abs_ms", 30.0),
                        concurrency_limit_enabled=config.get("concurrency_limit_enabled", False),
                        concurrency_limit_initial=config.get("concurrency_limit_initial", 16),
                        concurrency_limit_min=config.get("concurrency_limit_min", 2),
                        concurrency_limit_max=config.get("concurrency_limit_max", 128),
                        concurrency_add_on_success=config.get("concurrency_add_on_success", 4),
                        concurrency_mult_on_failure=config.get("concurrency_mult_on_failure", 0.5),
                        concurrency_failure_window=config.get("concurrency_failure_window", 20),
                        conn_pool_enabled=config.get("conn_pool_enabled", False),
                        conn_pool_per_proxy=config.get("conn_pool_per_proxy", 4),
                        conn_pool_total=config.get("conn_pool_total", 64),
                        conn_pool_idle_timeout=config.get("conn_pool_idle_timeout", 30.0),
