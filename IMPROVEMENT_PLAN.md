...
| **P1 ✅ 已完成** | **Cost 分解输出暴露**：`get_pid_quality_v2`（即 `/quality/meta`）每代理增 `cost_breakdown`（三分量 raw/norm/contrib + load_mult + rank + slow-start/未知标记；非候选=熔断/禁用/并发超限为 None）；`test_routing.py --metrics` 每代理一行紧凑分解 | §六.3 刻意留到观测期的项，已落地。贡献最大的分量即当前排序主导因素——直接回答"该调哪个权重"。纯观测增强，不改路由行为。（按域名的分解未做，全局视角已够定位主导权重；需要时按同模式扩展 `get_domain_metrics`） |
