...
- **`conn_pool.refill_pause_silence_sec`** (default 120): activity-detection silence window (seconds). Production found background heartbeats (GitHub Desktop's `alive.github.com`, Windows' `client.wns.windows.com`, Edge cloud-messaging — every 3-10 min) keep resetting "time since last request", so `refill_pause_minutes` never fired. Activity is now judged as **dense requests**: only a request arriving within this window of the last one refreshes the timestamp; more-isolated requests (heartbeats) don't, so they can't defeat the idle pause. Set `0` to keep the old refresh-on-any-request behavior.
