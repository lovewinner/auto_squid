...
router:
  ...
  # conn_pool: {enabled: true, per_proxy: 4, total: 64, idle_timeout: 30.0, refill_interval: 5.0, refill_target: 2, connect_timeout: 10.0, target_prewarm: true, refill_pause_minutes: 60, refill_pause_heartbeat_targets: [".github.com", ".wns.windows.com", ".cloudmessaging.edge.microsoft.com", ".edge.microsoft.com"], established_reuse: true}
logging:
  file: "auto_squid.log"
```
...
- **`conn_pool.refill_pause_heartbeat_targets`** (default: built-in list of common background-push domain suffixes): activity is judged **per target**. Production found background heartbeats (GitHub Desktop's `alive.github.com`, Windows' `client.wns.windows.com`, Edge cloud-messaging — every 3-10 min) keep resetting "time since last request", so `refill_pause_minutes` never fired. Targets matching these suffixes are treated as heartbeats and don't refresh the activity timestamp, so they can't defeat the idle pause; **any other target refreshes regardless of interval — real isolated requests are not wrongly ignored** (the earlier `silence_sec` interval-based cutoff did ignore them). Empty list = no per-target filter (any request refreshes).
