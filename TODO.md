...
- [x] CONNECT 上游 TCP 预热池（conn_pool）：每代理维护空闲连接，省建连 TTFB
  ...
  - [x] 第四阶段·请求簇预测预热（conn_pool.cluster_predict，2026-08-27）：`cluster.py` `ClusterGraph` 协作类——per-client 页面加载窗口（默认 2s）内 CONNECT target 成簇，关窗即学进**全局共现图**（`src_target → {co_target: {hits, probe_pid, last_seen}}`，TTL + LRU 上限，跨客户端共享冷启动继承）；下次窗口开口首个带赢家请求即预测 top-K co-target，经注入 `_spawn_target_prewarm` 预建"本机→上游代理"裸 TCP（不 CONNECT 源站，错预建 30s 空闲淘汰、共享 conn_pool_total 预算），取用阶梯在 target 池复用。门控 `enabled + target_prewarm`，双 6 配置字段（topk/support/ttl/max_entries/throttle 内置钳制），观察点收进 `_handle_connect` 三分支，flush 周期 prune。**211 全过**（新增 11 个：窗口分组/共现学习/支持度门/跳过窗内/throttle/LRU 上限/TTL+陈旧窗口关窗/快照/e2e 学习后预测落地 target 池 + 2 个配置依赖校验）
...
- [x] #14 续拆分（2026-08-27）：`selector.py` 纯平移（ProxySelector + 5 常量，零 body 改动）+ `http_cache.py`（`HttpCache` 协作类：LRU 层 + 在途聚合 + 门 + 缓冲上限）+ `sticky.py`（`StickyCache` 协作类，背引用 router 读决策链，共享 `_degraded_single_send`）。Router 缩掉 ~744 行，经 `_CACHE_FORWARD`/`_STICKY_FORWARD` 白名单 `__getattr__/__setattr__` 转发（三张名单两两不相交）。domain_cache 簇（`_meta_cache`/adaptive-ttl/switch-damping/single-send-degrade/DB/policies）仍留 Router（紧耦合决策链 + api.py 直读 `_router._policies`）。**零测试行改动,200 全过**
