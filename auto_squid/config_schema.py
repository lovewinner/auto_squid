...
class ConnPoolConfig(ConfigBase):
    ...
    prehandshake_throttle_window_sec: float = Field(0.0, description="预握手节流窗口(秒):滑动窗口内最多发射 prehandshake_throttle_max_per_window 条预握手,超出静默跳过并计 prehandshake_throttled_skips。生产事故(09-01):页面首载瞬间粘性命中对几十个 target × 多代理疯狂预握手,3 秒内 PREHANDSHAKE 落池 20+,把上游代理压到过载→全量熔断风暴。0=不限速(默认,零行为变化)")
    prehandshake_throttle_max_per_window: int = Field(0, description="预握手节流窗口内发射上限(条),见 prehandshake_throttle_window_sec。≤0=不限速(默认)。仅当 window_sec>0 且本值>0 时节流生效")
...
class RouterConfig(ConfigBase):
    ...
    stagger_initial: int = Field(2, description="错峰首批并发数(延迟优先默认双发)")
    stagger_interval_ms: int = Field(100, description="错峰启动间隔(毫秒),钳制到[100,2000]")
